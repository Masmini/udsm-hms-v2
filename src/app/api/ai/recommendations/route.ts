...
export async function POST(request: NextRequest) {
  try {
    ...
    const recommendations = await generatePersonalizedRecommendations(user, context, limit);
...
async function generatePersonalizedRecommendations(
  ...
  const eventRecs = await generateEventRecommendations(user, userSkills, userHubs);
  recommendations.push(...eventRecs);

  // Project Recommendations
  const projectRecs = await generateProjectRecommendations(user, userSkills, userHubs);
  recommendations.push(...projectRecs);

  // Programme Recommendations
  const programmeRecs = await generateProgrammeRecommendations(user, userSkills, userProgrammes);
  recommendations.push(...programmeRecs);

  // Hub Recommendations
  const hubRecs = await generateHubRecommendations(user, userSkills, userInterests, userHubs);
  recommendations.push(...hubRecs);

  // User/Collaboration Recommendations
  const userRecs = await generateUserRecommendations(user, userSkills, userHubs);
...
async function generateEventRecommendations(
  ...
  const availableEvents = upcomingEvents.filter(event => event.registrations.length === 0);
  ...
  for (const event of availableEvents) {
    ...
    if (event.eventType.toLowerCase().includes('workshop') || 
...
async function generateProjectRecommendations(
  ...
  for (const project of availableProjects) {
    ...
    if (skillMatch > 0) {
      ...
      reason = `Perfect match for your skills: ${userSkills.slice(0, 2).join(', ')}`;
    }
...
async function generateProgrammeRecommendations(
  ...
  for (const programme of availableProgrammes) {
    ...
    const daysUntilStart = Math.floor(
      (new Date(programme.startDate!).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    );
...
async function generateHubRecommendations(
  ...
  for (const hub of availableHubs) {
    ...
    if (interestMatch > 0) {
      ...
      reason = `Matches your interests in ${hubCategories.slice(0, 2).join(', ')}`;
    }

    // Boost confidence based on hub activity
    const totalActivity = hub._count.projects + hub._count.events + hub._count.programmes;
    if (totalActivity > 5) {
      confidence += 0.2;
  ...
  for (const otherUser of similarUsers) {
    ...
    if (confidence > 0.4) {
      recommendations.push({
        ...
        description: `${otherUser.degreeProgramme || 'Student'} with expertise in ${otherUserSkills.slice(0, 2).join(', ')}`,
        confidence: Math.min(confidence, 0.95),
        reason,
        data: otherUser,
      });
