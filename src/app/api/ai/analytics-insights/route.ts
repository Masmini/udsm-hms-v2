...
export async function POST(request: NextRequest) {
  try {
    ...
    const insights = await generateAdvancedInsights(analyticsData, userRole, hubId);
...
function analyzeUserEngagement(engagement: AnalyticsData['userEngagement']): AIInsight[] {
  const insights: AIInsight[] = [];
  ...
  if (activeRatio < 30) {
    insights.push({
      ...
      description: `Only ${activeRatio.toFixed(1)}% of users are active. Many registered users are not engaging with the platform.`,
      confidence: 0.81,
      priority: 'medium',
      actionable: true,
      impact: 'neutral',
...
function analyzeHubPerformance(hubPerformance: AnalyticsData['hubPerformance'], userRole: string): AIInsight[] {
  const insights: AIInsight[] = [];
  const { totalHubs, activeHubs, averageMembers, topPerformingHubs } = hubPerformance;
  ...
  if (activeHubRatio < 70) {
    insights.push({
      ...
      description: `${100 - activeHubRatio}% of hubs are inactive or underperforming. Hub consolidation or revitalization needed.`,
      confidence: 0.76,
      priority: 'medium',
      actionable: true,
      impact: 'negative',
  ...
  if (topPerformingHubs.length > 0 && userRole === 'ADMIN') {
    insights.push({
      ...
      description: 'Analysis of top-performing hubs reveals best practices that can be replicated across the platform.',
      confidence: 0.87,
      priority: 'medium',
      actionable: true,
      impact: 'positive',
...
function analyzeProjectMetrics(projectMetrics: AnalyticsData['projectMetrics']): AIInsight[] {
  const insights: AIInsight[] = [];
  const { totalProjects, completedProjects, completionRate, averageCompletionTime } = projectMetrics;
  ...
  if (averageCompletionTime > 180) { // More than 6 months
    insights.push({
  ...
  if (projectsPerHub < 2) {
    insights.push({
      ...
      description: 'Few projects are being initiated. This may indicate barriers to project creation or lack of innovation culture.',
      confidence: 0.71,
      priority: 'medium',
      actionable: true,
      impact: 'negative',
...
function analyzeEventMetrics(eventMetrics: AnalyticsData['eventMetrics']): AIInsight[] {
  const insights: AIInsight[] = [];
  const { totalEvents, upcomingEvents, averageAttendance, popularEventTypes } = eventMetrics;
  ...
  if (popularEventTypes.length > 0) {
    insights.push({
      ...
      description: 'User preferences for specific event types can guide future event planning and resource allocation.',
      confidence: 0.86,
      priority: 'low',
      actionable: true,
      impact: 'positive',
      recommendations: [
        `Focus on popular event types: ${popularEventTypes.slice(0, 3).join(', ')}`,
...
function analyzeCrossMetricCorrelations(data: AnalyticsData): AIInsight[] {
  ...
  if (avgHubSize > 15 && projectSuccess > 70) {
    insights.push({
      ...
      description: 'Larger hubs (15+ members) show significantly higher project completion rates. This suggests optimal hub sizing strategies.',
      confidence: 0.83,
      priority: 'medium',
      actionable: true,
      impact: 'positive',
  ...
  if (engagement < 40 && eventAttendance < 50) {
    insights.push({
      ...
      description: 'Users with low platform engagement are also less likely to attend events, suggesting a compound engagement problem.',
      confidence: 0.77,
      priority: 'high',
      actionable: true,
      impact: 'negative',
...
function generatePredictiveInsights(data: AnalyticsData): AIInsight[] {
  ...
  if (isAcademicSeason) {
    insights.push({
      ...
      description: 'Historical patterns suggest 40-60% increase in platform activity during academic months. Infrastructure scaling recommended.',
      confidence: 0.91,
      priority: 'medium',
      actionable: true,
      impact: 'positive',
  ...
  if (engagementTrend < -2) {
    insights.push({
      ...
      description: 'Declining engagement trends suggest potential user churn in the next 30-60 days without intervention.',
      confidence: 0.74,
      priority: 'high',
      actionable: true,
      impact: 'negative',
  ...
  if (usersPerHub > 50) {
    insights.push({
      ...
      description: 'High user-to-hub ratio suggests demand for additional specialized hubs. Strategic expansion could capture unmet needs.',
      confidence: 0.68,
      priority: 'low',
      actionable: true,
      impact: 'positive',
